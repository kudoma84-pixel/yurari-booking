...
export default function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const searchRes = await fetch(`${SUPABASE_URL}/rest/v1/customers?tel=eq.${encodeURIComponent(form.tel)}&select=id`, { headers });
      ...
      if (customers && customers.length > 0) {
        ...
        const newRes = await fetch(`${SUPABASE_URL}/rest/v1/customers`, {
          method: "POST", headers,
          body: JSON.stringify({ name: form.name, kana: form.kana, tel: form.tel, email: form.email, points: 0 }),
        });
      ...
      await fetch(`${SUPABASE_URL}/rest/v1/bookings`, {
        method: "POST", headers,
        body: JSON.stringify({
          customer_id: customerId, store_id: store.id, course_id: course.id,
          course_name: course.name, staff_id: staff.id, staff_name: staff.name,
          booking_date: formatDate(date), booking_time: time,
          status: "confirmed", notes: form.notes, booking_number: num,
        }),
      });
      setBookingNum(num);
      setSubmitted(true);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #fdf9f4 0%, #f5ede0 50%, #edf5f0 100%)", fontFamily: "sans-serif" }}>
      <div style={{ background: "rgba(255,255,255,0.85)", borderBottom: "1px solid #e8d9c8", padding: "16px 20px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#3a5a3a" }}>整体院 癒楽里</div>
            <div style={{ fontSize: 11, color: "#7a9a7a" }}>YURARI SEITAI-IN</div>
          </div>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 16px 80px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              {steps.slice(0,4).map((s,i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", flex: i < 3 ? 1 : "none" }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0, background: i < step ? "#5a9e7a" : i === step ? "#3a5a3a" : "#e0d5c8", color: i <= step ? "white" : "#999" }}>{i < step ? "✓" : i+1}</div>
        ...
        {step === 0 && (
          <div style={{ paddingTop: 24 }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#3a5a3a", marginBottom: 20 }}>店舗を選んでください</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {STORES.map(s => (
                <div key={s.id} onClick={() => setStore(s)} style={{ background: store?.id === s.id ? "linear-gradient(135deg, #eaf5ec, #e0f0e8)" : "white", border: `2px solid ${store?.id === s.id ? "#5a9e7a" : "#e8ddd0"}`, borderRadius: 16, padding: "20px 24px", cursor: "pointer" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ fontSize: 32 }}>🏥</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 17, fontWeight: 700, color: "#3a5a3a" }}>癒楽里 {s.name}</div>
                      <div style={{ fontSize: 12, color: "#7a9a7a" }}>{s.address}</div>
                      <div style={{ fontSize: 12, color: "#7a9a7a" }}>📞 {s.tel}</div>
                      <div style={{ fontSize: 11, color: "#aaa" }}>🕐 {s.hours}</div>
                    </div>
        ...
        {step === 1 && (
          <div style={{ paddingTop: 24 }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#3a5a3a", marginBottom: 20 }}>コースを選んでください</h2>
              ...
              {COURSES.map(c => (
                <div key={c.id} onClick={() => setCourse(c)} style={{ background: course?.id === c.id ? "linear-gradient(135deg, #eaf5ec, #e0f0e8)" : "white", border: `2px solid ${course?.id === c.id ? "#5a9e7a" : "#e8ddd0"}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#3a5a3a" }}>{c.name}</div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {[{ id: "any", name: "指名なし", title: "", specialty: "おまかせ", img: "🌿" }, ...(STAFFS[store?.id] || [])].map(s => (
                  <div key={s.id} onClick={() => setStaff(s)} style={{ background: staff?.id === s.id ? "linear-gradient(135deg, #eaf5ec, #e0f0e8)" : "white", border: `2px solid ${staff?.id === s.id ? "#5a9e7a" : "#e8ddd0"}`, borderRadius: 12, padding: "12px 16px", cursor: "pointer", textAlign: "center", minWidth: 100 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#3a5a3a" }}>{s.name}</div>
                    <div style={{ fontSize: 10, color: "#aaa" }}>{s.specialty}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {dates.map((d, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setDate(d)} style={{ background: isSelected ? "linear-gradient(135deg, #5a9e7a, #3a7a5a)" : "white", border: `2px solid ${isSelected ? "#5a9e7a" : "#e8ddd0"}`, borderRadius: 12, padding: "10px 12px", cursor: "pointer", textAlign: "center", flexShrink: 0, minWidth: 52 }}>
                      <div style={{ fontSize: 10, color: isSelected ? "rgba(255,255,255,0.8)" : "#aaa" }}>{DAYS_JP[d.getDay()]}</div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {TIME_SLOTS.map(t => {
                    ...
                    return (
                      <div key={t} onClick={() => setTime(t)} style={{ background: isSelected ? "linear-gradient(135deg, #5a9e7a, #3a7a5a)" : "white", border: `2px solid ${isSelected ? "#5a9e7a" : "#e8ddd0"}`, borderRadius: 10, padding: "8px 14px", cursor: "pointer", fontSize: 13, fontWeight: 600, color: isSelected ? "white" : "#3a5a3a" }}>{t}</div>
                    );
                  })}
                </div>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type || "text"} value={form[f.key]} onChange={e => setForm({ ...form, [f.key]: e.target.value })} placeholder={f.placeholder} style={{ width: "100%", padding: "12px 16px", borderRadius: 12, border: "2px solid #e8ddd0", fontSize: 15, color: "#3a5a3a", background: "white", boxSizing: "border-box", outline: "none" }} />
                </div>
              ))}
              <div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="肩こりがひどく..." rows={3} style={{ width: "100%", padding: "12px 16px", borderRadius: 12, border: "2px solid #e8ddd0", fontSize: 14, color: "#3a5a3a", background: "white", boxSizing: "border-box", outline: "none", resize: "none", fontFamily: "inherit" }} />
              </div>
            </div>
          </div>
        )}
        {step === 4 && !submitted && (
          <div style={{ paddingTop: 24 }}>
            ...
            <button onClick={handleSubmit} disabled={loading} style={{ width: "100%", padding: "16px", borderRadius: 14, border: "none", cursor: "pointer", background: "linear-gradient(135deg, #5a9e7a, #3a7a5a)", color: "white", fontSize: 16, fontWeight: 700 }}>
        ...
        {step === 4 && submitted && (
          <div style={{ paddingTop: 40, textAlign: "center" }}>
            ...
            <div style={{ background: "linear-gradient(135deg, #eaf5ec, #e0f0e8)", borderRadius: 16, padding: "20px 24px", marginBottom: 24, display: "inline-block", minWidth: 240 }}>
              ...
              <div style={{ fontSize: 24, fontWeight: 700, color: "#3a5a3a" }}>{bookingNum}</div>
            </div>
            <div style={{ fontSize: 12, color: "#aaa", marginBottom: 20, lineHeight: 1.8 }}>当日は予約時間の5分前にお越しください。<br/>キャンセル・変更は前日17時まで承ります。</div>
            <a href={store?.lineUrl} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, width: "100%", padding: "15px", borderRadius: 14, background: "#06C755", color: "white", fontSize: 15, fontWeight: 700, textDecoration: "none", marginBottom: 12, boxSizing: "border-box" }}>LINEでお問い合わせ・変更はこちら</a>
        ...
        {!submitted && (
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(255,255,255,0.95)", borderTop: "1px solid #e8ddd0", padding: "12px 16px" }}>
