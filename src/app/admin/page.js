          ...
          <div style={{ background: "white", borderRadius: 20, padding: 32, width: "100%", maxWidth: 480, boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              {["confirmed","received","treatment_done","cancelled"].map(s => (
               <button key={s} onClick={async () => { await updateBookingStatus(selectedBooking.id, s); if (s === "received" && selectedBooking.customer_id) { const r = await fetch(SUPABASE_URL + "/rest/v1/customers?id=eq." + selectedBooking.customer_id, { headers }); const d = await r.json(); if (d[0]?.customer_number) alert("受付完了！顧客番号：" + d[0].customer_number); } }} disabled={selectedBooking.status === "received" && s === "received"} style={{ flex: 1, padding: "10px", borderRadius: 10, border: "2px solid " + statusColor(s), background: selectedBooking.status === s ? statusColor(s) : "white", color: selectedBooking.status === s ? "white" : statusColor(s), fontSize: 11, fontWeight: 600, cursor: selectedBooking.status === "received" && s === "received" ? "not-allowed" : "pointer", opacity: selectedBooking.status === "received" && s === "received" ? 0.5 : 1 }}>{statusLabel(s)}</button>
              ))}
            </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {!editingCustomer && (
            ...
            <button onClick={() => { const win = window.open('', '_blank'); const sName = currentStore?.name || '南浦和店'; const cNum = selectedCustomer.customer_number || ''; const cName = selectedCustomer.name || ''; const qUrl = 'https://yurari-booking.vercel.app/admin?checkin=' + cNum; const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>会員カード</title><script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"><\/script><style>@page{size:85.6mm 54mm;margin:0}body{width:85.6mm;height:54mm;margin:0;padding:0;font-family:sans-serif}.card{width:85.6mm;height:54mm;background:white;display:flex;flex-direction:column;justify-content:space-between;padding:4mm}.store{font-size:8pt;color:#2d6a4f;font-weight:700}.name{font-size:11pt;font-weight:700}.number{font-size:8pt;color:#666}.bottom{display:flex;justify-content:space-between;align-items:flex-end}#qr canvas,#qr img{width:18mm!important;height:18mm!important}<\/style><\/head><body><div class="card"><div class="store">整体院 癒楽里 ' + sName + '<\/div><div class="name">' + cName + '<\/div><div class="number">会員番号：' + cNum + '<\/div><div class="bottom"><div style="font-size:7pt;color:#aaa">yurari-booking.vercel.app<\/div><div id="qr"><\/div><\/div><\/div><script>new QRCode(document.getElementById("qr"),{text:"' + qUrl + '",width:68,height:68});setTimeout(()=>window.print(),800);<\/script><\/body><\/html>'; win.document.write(html); win.document.close(); }} style={{ padding: "6px 16px", borderRadius: 8, border: "2px solid #e07b39", background: "white", color: "#e07b39", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>🖨 カード印刷</button>
          )}
