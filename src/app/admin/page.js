  ...
  const fetchMonthlyReport = async (monthStr) => {
    ...
    try {
      ...
      const payToJstMonth = (p) => {
        ...
        const hasTimezone = p.created_at.endsWith('Z') || p.created_at.indexOf('+') > 0;
        const s = hasTimezone ? p.created_at : p.created_at + 'Z';
          ...
          <div style={{ background: "white", borderRadius: 20, padding: 32, width: "100%", maxWidth: 480, maxHeight: "90vh", overflow: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                ...
                <input value={directBookingForm.tel || ""} onChange={e => setDirectBookingForm(f => ({ ...f, tel: e.target.value.split("").filter(c => (c >= "0" && c <= "9") || c === "-").join("") }))} placeholder="090-1234-5678" inputMode="tel" style={{ width: "100%", padding: "10px 16px", borderRadius: 10, border: "2px solid #e8ddd0", fontSize: 14, boxSizing: "border-box" }} />
              </div>
              <div>
